...
class dias2010 :
    ...
    def sort(self) :
        ...
        while not graph.is_ordered() :#
            try :
                distance = distance + 1
                (i,j,k) = self.sort_steps(graph)
                sequence.append([i,j,k])
                graph.transposition(i,j,k)
            except :
                print "Erro %s" % graph.__str__()
                print "dias2010_1.5 - %s - %i - %s" % (self.input, distance, sequence)
                sys.exit()

        print "dias2010_1.5 - %s - %i - %s" % (self.input, distance, sequence)

    def sort_steps(self, cycle_graph, transposition = True) :
        def call_oriented(graph, function, cycles, d_triples, need_triples, zero_move, mode, transposition) :
            if mode :
                return call_oriented2(graph, function, cycles, d_triples, 
                                      need_triples, zero_move, transposition)
            else :
                return call_oriented1(graph, function, cycles, d_triples, 
                                      need_triples, zero_move, transposition)

        def call_oriented1(graph, function, cycles, d_triples, need_triples, zero_move, transposition) :
            for cycle_count in range(0,len(cycles)) :
                cycle = cycles[cycle_count]
                cycle_count = cycle_count + 1
                answer = False
                if need_triples :
                    if not d_triples.has_key(cycle.ncycle) :
                        d_triples[cycle.ncycle]=graph.find_oriented_triple(cycle,all=True,zero_move=zero_move)
                    answer = function(cycle, d_triples[cycle.ncycle], transposition)
                else :
                    answer = function(cycle, transposition)
                if transposition :
                    (i,j,k) = answer
                    if i and j and k :
                        return (i,j,k)
        ...
        def call_oriented2(graph, function, cycles, d_triples, need_triples, zero_move, transposition):
            for cycle_count_1 in range(0,len(cycles)) :
                for cycle_count_2 in range(0, len(cycles)) :
                    if cycle_count_1 != cycle_count_2 :
                        cycle_1 = cycles[cycle_count_1]
                        cycle_2 = cycles[cycle_count_2]
                        if not d_triples.has_key(cycle_1.ncycle) :
                            d_triples[cycle_1.ncycle]=graph.find_oriented_triple(cycle_1,all=True,
                                                                                 zero_move=zero_move)
                        if not d_triples.has_key(cycle_2.ncycle) :
                            d_triples[cycle_2.ncycle]=graph.find_oriented_triple(cycle_2,all=True,
                                                                                 zero_move=zero_move)

                        answer = function(d_triples[cycle_1.ncycle],d_triples[cycle_2.ncycle],transposition)
                        if transposition :
                            (i,j,k) = answer
                            if i and j and k :
                                return (i,j,k)
        ...
        # Oriented cycle
        cycles = cycle_graph.get_oriented_cycles()
        if len(cycles) > 0 :
            oriented_functions = [[cycle_graph.oriented_crossed_transposition,1,0,0],
                                  [cycle_graph.strongly_oriented_transposition,0,0,0],
                                  [cycle_graph.oriented_even_transposition,1,0,0],
                                  [cycle_graph.oriented_inner_non_oriented_transposition,1,0,0],
                                 # [cycle_graph.oriented_generic_transposition,1,0,0],
                                  [cycle_graph.oriented_zero_move_crossed_even_transposition,1,1,0],
                                  [cycle_graph.oriented_zero_move_crossed_oriented_transposition1,1,1,1],
                                  [cycle_graph.oriented_zero_move_crossed_oriented_transposition2,1,1,1],
                                  [cycle_graph.oriented_zero_move_generic_transposition,1,1,0]
                                  ]
            #call functions
            dict_triples = {}
            for count in range(0,len(oriented_functions)) :
                if count == 4 :
                    dict_triples = {}
                func = oriented_functions[count]
                answer = call_oriented(cycle_graph, func[0], cycles, dict_triples, 
                                        func[1], func[2],func[3], transposition)
                if transposition :
                    (i,j,k) = answer 
                    if i and j and k:
                        self.trace = self.trace + str(count) + ","
            ...
            if not transposition :
                return 10 #Apenas um numero altinho 

            triples = cycle_graph.find_non_oriented_triples()
            if len(triples) > 0 :
                for triple in triples :
                    (i,j,k) = triple
                    next_step = self.analyze_transposition(cycle_graph, i.index, j.index, k.index)
                    triple.append(next_step)
                
                menor = triples[0]
                for triple in triples :
                    if triple[3] < menor[3] :
                        menor = triple
                self.trace = self.trace + "11,"
                return menor[0].index, menor[1].index, menor[2].index

        cycle = cycle_graph.get_long_non_oriented_cycle()
        if cycle :
